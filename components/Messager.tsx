...
export class Messager extends React.Component<
  MessagerProps,
  { messages: string[]; waiting: Boolean }
> {
  ...
  async addMessage(message: string) {
    if (!this.state.waiting) {
      ...
      this.setState({ messages: messages, waiting: true});
      try {
        ...
        this.setState({ messages: messages, waiting: false});
    ...
    return (
      <>
        <Messages
          ...
          onChange={((i: number, e: React.FormEvent<HTMLDivElement>) => {
